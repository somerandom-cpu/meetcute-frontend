...
const VerifyEmailPage = () => {
  ...
  useEffect(() => {
    ...
    // Prevent double invocation in React StrictMode (dev) or accidental multiple calls
    if (hasCalledRef.current) {
      return; // already initiated verification
    }
    ...
    const verifyToken = async () => {
      try {
        ...
        // Call the verify email API with error handling
        ...
        // If we have a token in the response, log the user in
        ...
        // If the error indicates the email is already verified
        if (errorMessage.toLowerCase().includes('already verified') || 
            (error.response?.status === 400 && errorMessage.toLowerCase().includes('invalid'))) {
          setStatus('success');
